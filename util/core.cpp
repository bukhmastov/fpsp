...
void Core::generate(unsigned int seed) {
    ...
    questions.push_back(new Question("Определить значение ненормированной периодической ВКФ R12(m) двоичных ПСП А1={a0 a1 a1 a0 a1 a0 a0}, А2={a1 a0 a1 a1 a0 a0 a1} для сдвига m=0 в пространстве Евклида", std::vector<QString>{"R12(0)=-3", "R12(0)=-1", "R12(0)=+1", "R12(0)=+3"}));
    questions.push_back(new Question("Определить значение ненормированной периодической ВКФ R12(m) троичных ПСП А1={a0 a1 a2 a0}, А2={a1 a0 a0 a2} для сдвига m=0 в пространстве Евклида (a0=1, a1=exp(j2п/3), a2=exp(j4п/3))", std::vector<QString>{"R12(0)=-2", "R12(0)=-1", "R12(0)= 0", "R12(0)=+1"}));
    questions.push_back(new Question("Определить расстояние между двумя последовательностями в метрике Хэмминга А1={0110100}, А2={0101101} для сдвига m=0", std::vector<QString>{"D12=3", "D12=1", "D12=2", "D12=0"}));
    questions.push_back(new Question("Определить в метрике Ли расстояние между двумя 5-ричными последовательностями А1={0140302}, А2={1403101} для сдвига m=0", std::vector<QString>{"D12=9", "D12=8", "D12=7", "D12=10"}));
    questions.push_back(new Question("Определить значение ненормированной периодической ВКФ R12(m) троичных ПСП с периодом N=7 А1={1 1 2 1 2 1 0}, А2={1 0 2 1 0 2 0} для сдвига m=0 в пространстве Ли", std::vector<QString>{"R12(0)=3", "R12(0)=2", "R12(0)=1", "R12(0)=-1"}));
    questions.push_back(new Question("Определить значение ненормированной периодической ВКФ R12(m) двоичных ПСП с периодом N=7 А1={1 1 0 1 0 1 0}, А2={1 0 0 1 1 1 0} для сдвига m=0 в пространстве Хэмминга", std::vector<QString>{"R12(0)=3", "R12(0)=4", "R12(0)=2", "R12(0)=1"}));
    ...
    questions.push_back(new Question("Определение М-последовательности с периодом N=63", std::vector<QString>{"Это двоичная ПСП с периодом N = 2^6–1, построенная над полем Галуа GF(2^6) и имеющая двухуровневую ПАКФ", "Это двоичная ПСП с периодом N = 2^4–1, построенная над полем Галуа GF(2^4) и имеющая одноуровневую ПАКФ", "Это двоичная ПСП с периодом N = 2^6–1, построенная над полем Галуа GF(2^4) и имеющая двухуровневую ПАКФ", "Это двоичная ПСП с периодом N = 2^6–1, построенная над полем Галуа GF(2^6) и имеющая трехуровневую ПАКФ"}));
    questions.push_back(new Question("В поле Галуа характеристики p=3 задан проверочный примитивный полином h(x)=x^4+2x^3+x^2+2. Чему равен период формируемой ПСП?", std::vector<QString>{"N=80", "N=40", "N=20", "N=32"}));
    questions.push_back(new Question("В поле Галуа характеристики p=3 задан проверочный примитивный полином h(x)=x^4+2x^3+x^2+2. Сколько сумматоров по модулю 3 содержит схема формирования МП?", std::vector<QString>{"2", "1", "3", "4"}));
    questions.push_back(new Question("В поле Галуа характеристики p=3 задан проверочный примитивный полином h(x)=x^4+2x^3+x^2+2. Сколько умножителей содержит схема формирования МП?", std::vector<QString>{"3", "4", "2", "1"}));
    ...
    questions.push_back(new Question("Над полем GF(2^3) сформировать М-последовательность для примитивного полинома h(x)=x^3+x^2+1 для начального состояния С0 = 1, С1 = 1, С2 = 1", std::vector<QString>{"1110100", "1110010", "1110101", "1110011"}));
    questions.push_back(new Question("Над полем GF(3^2) сформировать троичную МП для примитивного полинома h(x)=x^2+2x+2 при условии С0 = 1, С1 = 1", std::vector<QString>{"11202210", "11021201", "11201022", "11021110"}));
    ...
    questions.push_back(new Question("Полиномы h1(x) и h2(x) задают предпочтительную пару М-последовательностей с периодом N=63, элемент а^5 поля GF(2^6) – один из корней полинома h1(x). Определить корень полинома h2(x).", std::vector<QString>{"а^22", "а^20", "а^18", "а^15"}));
    questions.push_back(new Question("Полиномы h1(x) и h2(x) являются сомножителями проверочного полинома малого множества Касами с периодом N=63, элемент а^5 поля GF(2^6) – один из корней примитивного полинома h1(x). Определить корень полинома h2(x).", std::vector<QString>{"а^45", "а^54", "а^55", "а^35"}));
